...
foreach ($posts as $post):
	...
	?>
		...
		<div class="item">
			<a href="/user/<?= $post->uid ?>/" class="ui tiny image">
			...
			<div class="content">
				...
				<div class="meta" style="display:inline;">
					<a href="/user/posts/<?= $post->id; ?>/" class="time" id="post-date"><?= $post->date ?></a>
				</div>
				...
				<div class="extra">
					...
					<a href="/user/posts/<?= $post->id; ?>/" style="cursor:pointer;" class="comments"><i class="comments blue icon"></i> <?= $cCount ?> comments</a>
				</div>
			</div>
		</div>
	<?php
