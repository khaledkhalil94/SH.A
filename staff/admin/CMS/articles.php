...
switch ($display) {
	case 'pub':
		$sortby = "created DESC";
		$articles = Admin::get_articles(1);
		$Count = count($articles);
		break;
	case 'unpub':
		$sortby = "status desc";
		$articles = Admin::get_articles(2);
		$Count = count($articles);
		break;
	case 'del':
		$articles = Admin::get_del_articles(3);
		$Count = count($articles);
			...
			<p>Display:</p>
				<nav>
					<ul class="pager">
						...
						<li><a href="?display=pub"><?= $greenIcon; ?> Published</a></li>
						<li><a href="?display=unpub"><?= $greyIcon; ?> unPublished</a></li>
						<li><a href="?display=del"><?= $redIcon; ?> Marked for deletion</a></li>
					</ul>
				</nav>
			</div>
			<hr>
				...
				<tbody>
					...
					<tr>
						<td >
							<p><a href="article.php?id=<?= $id; ?>"><b><?= $title; ?></b></a></p>
